...
testNumbers = do
    ...
    parseMatch jsonArray  "[0]"    
    parseMatch jsonArray  "[1,2,3,0.01]"    
...
testParseScientific q r = do
    let Right (Number n) = parseOnly json q
    if n == r
        then print "Ok."
        else print $ "Expected " ++ unpack q ++ " == " ++ show r ++ " (Result: " ++ show n ++ ")."

testScientific = do
    testParseScientific "-1.3e2" (-130)
    testParseScientific "1.1111e4" 11111
    testParseScientific "18181E-4" 1.8181
