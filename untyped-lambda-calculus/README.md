...
Notice that the lambda expression parser takes the parser for the abstraction body as an argument. Technically it wouldn't be a [combinator](https://wiki.haskell.org/Combinator) otherwise, but more importantly, this makes it easier to think about this function on its own &ndash; in particular since we haven't implemented the parser for the term itself yet.
